...
import {QuestionnaireListModel} from "../../core/models/questionnaire";

@Component({
  selector: 'app-doctor-questionnaires',
  templateUrl: './doctor-questionnaires.component.html',
  styleUrls: ['./doctor-questionnaires.component.css']
})
export class DoctorQuestionnairesComponent implements OnInit {
  enditem: any
  // bread crumb items
  ...
  ngOnInit() {
    ...
    this.doctorSrv.getAllQuestionnaires(doctorId).subscribe(x => {
  ...
  deleteRecord() {
    ...
    this.doctorSrv.deleteQuestionnaireById(this.deleteId).subscribe(x => {
      if (x) {
        Swal.fire({
          icon: 'success',
  ...
  editRecord(id: number) {
    this.router.navigate(['doctor/questionnaire-editor', id])
  }
